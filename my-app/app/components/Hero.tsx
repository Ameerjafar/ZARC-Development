            ...
            {steps.map((step, idx) => (
              ...
              >
                <div className="flex items-start gap-5">
                  <div
                    className={`mt-1 w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 shrink-0 ${
                      activeStep === idx
...
// --- 5. DATA PRECISION (UPDATED - NEURAL NETWORK VISUAL) ---
...
// --- NEW COMPONENT: Testimonials (Dribbble Style / Contained) ---
function Testimonials() {
  // Only 3 high-impact reviews as requested
  const reviews = [
    {
      name: "Rohan D.",
      role: "CEO, FinTech",
    ...
    <section className="py-24 bg-white border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mb-16 text-center">
          <span className="inline-block py-1 px-3 rounded-full bg-orange-50 text-orange-600 text-xs font-bold uppercase tracking-wider mb-4 border border-orange-100">
            Wall of Love
          </span>
        ...
        {/* Contained Slider Area */}
        <div className="relative w-full max-w-6xl mx-auto overflow-hidden rounded-[2.5rem] bg-slate-50 border border-slate-100 py-16">
          {/* Gradient Masks (To create the fade effect inside the box) */}
          ...
          {/* Scrolling Track */}
          <div className="flex gap-8 w-max animate-scroll-centered hover:[animation-play-state:paused] px-8">
            {/* We duplicate the array 3 times to ensure smooth infinite scrolling */}
            {[...reviews, ...reviews, ...reviews].map((review, idx) => (
              <div
                key={idx}
                className="w-[500px] p-10 rounded-[2rem] bg-white border border-slate-100 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.1)] transition-all duration-300 hover:-translate-y-1 group cursor-default"
              >
                <div className="flex justify-between items-start mb-6">
                  ...
                  <Quote className="w-10 h-10 text-slate-100 fill-slate-100 group-hover:text-orange-100 group-hover:fill-orange-50 transition-colors" />
                </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className="absolute inset-0 bg-orange-200 rounded-full blur opacity-40 group-hover:opacity-100 transition-opacity"></div>
                    <img
                      src={review.img}
                      alt={review.name}
      ...
      <style jsx>{`
        @keyframes scroll-centered {
          0% {
            transform: translateX(0);
          }
          100% {
            transform: translateX(-33.33%);
          } /* Move by 1/3rd because we tripled the array */
        }
        .animate-scroll-centered {
          ...
          <div className="grid lg:grid-cols-2 gap-12 p-12 md:p-20 relative z-10 items-center">
            ...
            <div className="space-y-8">
              ...
              <p className="text-slate-400 text-lg leading-relaxed max-w-md">
                Don't just dream about it. Get the validated roadmap, investor
