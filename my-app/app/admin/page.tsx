      ...
      <motion.aside initial={false} animate={{ width: isSidebarCollapsed ? 84 : 280 }} transition={{ type: "spring", stiffness: 260, damping: 28 }} className="h-full bg-white border-r border-slate-100 flex flex-col z-30 relative shadow-2xl shadow-slate-200/50">
        <div className="h-20 flex items-center px-6">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isSidebarCollapsed && (<motion.button onClick={() => router.push('/')} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} className="font-black text-xl tracking-tighter">Strat<span className="text-orange-500">Admin</span></motion.button>)}
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div>
              <button 
                ...
                className="group flex items-center gap-2 px-3 py-2 -ml-2 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-white hover:shadow-sm transition-all duration-200"
              >
                <div className="w-6 h-6 rounded-lg bg-white border border-slate-200 flex items-center justify-center group-hover:border-slate-300 shadow-sm">
